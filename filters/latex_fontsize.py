...
https://github.com/chdemko/pandoc-latex-fontsize/blob/develop/pandoc_latex_fontsize.py
"""
...
def fontsize(elem, doc):
    ...
    if doc.format in ("latex", "beamer") and elem.tag in (
        ...
        # debug(f"[INFO] pandoc-latex-fontsize: elem = {elem}")
        # debug(f"[INFO] pandoc-latex-fontsize: elem attributes = {elem.attributes}")
...
def add_definition(defined, definition):
    ...
    # debug("[INFO] pandoc-latex-fontsize: classes = " + str(classes))
    ...
    # debug("[INFO] pandoc-latex-fontsize: latex = " + latex_code(size))
